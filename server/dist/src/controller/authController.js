...
export const protect = catchAsync(async (req, res, next) => {
    ...
    if (!freshUser) {
        const message = new AllError(`the user who had this token no longer exist`, 401);
        return next(message);
    }
...
export const restriction = (...roles) => {
    return (req, res, next) => {
        ...
        if (!roles.includes(req.user.role)) {
            ...
            const message = new AllError(`you are trying to access an authorized page`, 403);
            return next(message);
        }
        next();
    };
};
export const forgotPasswords = catchAsync(async (req, res, next) => {
    ...
    try {
        ...
        await email.send("passwordReset", "Please reset your password in 10 minutes");
        res.status(200).json({
            message: "success",
        ...
        await user.save({ validateBeforeSave: false });
        return next(new AllError("password reset failed, please try again later", 500));
    }
});
...
export const updatePassword = catchAsync(async (req, res, next) => {
    ...
    const isPassword = await user.correctPassword(req.body.password, user.password);
    if (isPassword) {
